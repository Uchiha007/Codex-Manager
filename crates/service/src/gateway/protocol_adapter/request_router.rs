...
fn responses_content_part_to_anthropic(part: &Value) -> Result<Option<Value>, String> {
    ...
    match kind {
        ...
        "input_image" | "image" => Ok(responses_image_part_to_anthropic(obj)),
        _ => Ok(None),
    }
}
...
fn responses_image_part_to_anthropic(obj: &Map<String, Value>) -> Option<Value> {
    let image_url = obj
        ...
        .and_then(Value::as_str)
        .and_then(normalize_text)?;
    let (media_type, data) = parse_data_url_image(image_url.as_str())?;
    Some(json!({
        "type": "image",
        "source": {
            "type": "base64",
...
fn parse_data_url_image(value: &str) -> Option<(String, String)> {
    let rest = value.strip_prefix("data:")?;
    let (meta, data) = rest.split_once(',')?;
    if !meta.to_ascii_lowercase().ends_with(";base64") {
        return None;
    }
    let media_type = meta.trim_end_matches(";base64").trim();
